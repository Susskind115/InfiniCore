...
inline int chooseNumSplitsHeuristic(size_t num_heads, size_t num_seqs, size_t seqlen_k, int sm_count) {
    ...
    if (num_heads == 0 || num_seqs == 0) return 1;
    if (seqlen_k <= 256) return 1;
...
INFINIOP_CUDA_KERNEL flashAttentionDecodeHd128Warp(
    ...
    ptrdiff_t o_stride) {
    op::flash_attention::cuda::flashAttentionDecodeWarpKernel<Tindex, Tdata, 128>(
...
INFINIOP_CUDA_KERNEL flashAttentionDecodeHd128Cta(
    ...
    op::flash_attention::cuda::flashAttentionDecodeCtaKernel<Tindex, Tdata, 128, 64, 8>(
        out, q, k_cache, v_cache, block_tables, cache_lens, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size, q_stride,
        k_batch_stride, k_row_stride, k_head_stride, v_batch_stride, v_row_stride,
        v_head_stride, o_stride);
...
INFINIOP_CUDA_KERNEL flashAttentionDecodeHd128CtaTile16(
    ...
    ptrdiff_t v_head_stride,
    ptrdiff_t o_stride) {
    op::flash_attention::cuda::flashAttentionDecodeCtaKernel<Tindex, Tdata, 128, 64, 16>(
        out, q, k_cache, v_cache, block_tables, cache_lens, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size, q_stride,
        k_batch_stride, k_row_stride, k_head_stride, v_batch_stride, v_row_stride,
        v_head_stride, o_stride);
...
INFINIOP_CUDA_KERNEL flashAttentionDecodeHd128Cta32(
    ...
    op::flash_attention::cuda::flashAttentionDecodeCtaKernel<Tindex, Tdata, 128, 32, 8>(
        out, q, k_cache, v_cache, block_tables, cache_lens, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size, q_stride,
        k_batch_stride, k_row_stride, k_head_stride, v_batch_stride, v_row_stride,
        v_head_stride, o_stride);
...
INFINIOP_CUDA_KERNEL flashAttentionDecodeHd128Cta32Tile16(
    ...
    ptrdiff_t v_head_stride,
    ptrdiff_t o_stride) {
    op::flash_attention::cuda::flashAttentionDecodeCtaKernel<Tindex, Tdata, 128, 32, 16>(
        out, q, k_cache, v_cache, block_tables, cache_lens, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size, q_stride,
        k_batch_stride, k_row_stride, k_head_stride, v_batch_stride, v_row_stride,
        v_head_stride, o_stride);
...
INFINIOP_CUDA_KERNEL flashAttentionDecodeHd128CtaGqa4(
    ...
    op::flash_attention::cuda::flashAttentionDecodeCtaGqaKernel<Tindex, Tdata, 128, 64, 8, 4>(
        out, q, k_cache, v_cache, block_tables, cache_lens, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size, q_stride,
        k_batch_stride, k_row_stride, k_head_stride, v_batch_stride, v_row_stride,
        v_head_stride, o_stride);
...
INFINIOP_CUDA_KERNEL flashAttentionDecodeHd128SplitKv(
    ...
    ptrdiff_t v_head_stride,
    int num_splits) {
    op::flash_attention::cuda::flashAttentionDecodeSplitKvWarpKernel<Tindex, Tdata, 128>(
...
INFINIOP_CUDA_KERNEL flashAttentionDecodeHd128SplitKvCta(
    ...
    ptrdiff_t v_head_stride,
    int num_splits) {
    op::flash_attention::cuda::flashAttentionDecodeSplitKvCtaKernel<Tindex, Tdata, 128, 64, 8>(
        partial_acc, partial_m, partial_l,
        q, k_cache, v_cache, block_tables, cache_lens, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size, q_stride,
        k_batch_stride, k_row_stride, k_head_stride, v_batch_stride, v_row_stride,
...
INFINIOP_CUDA_KERNEL flashAttentionDecodeHd128SplitKvCtaTile16(
    ...
    ptrdiff_t v_head_stride,
    int num_splits) {
    op::flash_attention::cuda::flashAttentionDecodeSplitKvCtaKernel<Tindex, Tdata, 128, 64, 16>(
        partial_acc, partial_m, partial_l,
        q, k_cache, v_cache, block_tables, cache_lens, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size, q_stride,
        k_batch_stride, k_row_stride, k_head_stride, v_batch_stride, v_row_stride,
...
INFINIOP_CUDA_KERNEL flashAttentionDecodeHd128SplitKvCta32(
    ...
    ptrdiff_t v_head_stride,
    int num_splits) {
    op::flash_attention::cuda::flashAttentionDecodeSplitKvCtaKernel<Tindex, Tdata, 128, 32, 8>(
        partial_acc, partial_m, partial_l,
        q, k_cache, v_cache, block_tables, cache_lens, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size, q_stride,
        k_batch_stride, k_row_stride, k_head_stride, v_batch_stride, v_row_stride,
...
INFINIOP_CUDA_KERNEL flashAttentionDecodeHd128SplitKvCta32Tile16(
    ...
    ptrdiff_t v_head_stride,
    int num_splits) {
    op::flash_attention::cuda::flashAttentionDecodeSplitKvCtaKernel<Tindex, Tdata, 128, 32, 16>(
        partial_acc, partial_m, partial_l,
        q, k_cache, v_cache, block_tables, cache_lens, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size, q_stride,
        k_batch_stride, k_row_stride, k_head_stride, v_batch_stride, v_row_stride,
...
INFINIOP_CUDA_KERNEL flashAttentionDecodeHd128SplitKvCombine(
    ...
    int num_splits,
    ptrdiff_t o_stride) {
    op::flash_attention::cuda::flashAttentionDecodeSplitKvCombineWarpKernel<Tdata, 128>(
...
infiniStatus_t launch_decode_hd128_impl(
    ...
    auto dump_dispatch = [&](const char *path) {
        ...
        if (has_last &&
            cur.path == last.path &&
            cur.dtype == last.dtype &&
            cur.heads == last.heads &&
            cur.kv_heads == last.kv_heads &&
            cur.seqs == last.seqs &&
            cur.pbs == last.pbs &&
            cur.max_blocks == last.max_blocks &&
            cur.cta_tile == last.cta_tile &&
            cur.cta_threads == last.cta_threads &&
            cur.split == last.split &&
            cur.split_auto == last.split_auto &&
            cur.num_splits == last.num_splits &&
            cur.fixed == last.fixed &&
            cur.gqa_fused == last.gqa_fused) {
            return;
        }
        last = cur;
        has_last = true;
        ...
        fprintf(stderr,
                "[INFINIOP][flash_attention][hd128] dispatch: path=%s dtype=%d heads=%zu kv_heads=%zu seqs=%zu "
        ...
        if (const char *dbg = std::getenv("INFINIOP_FLASH_DEBUG_SPLITS")) {
            if (std::strcmp(dbg, "1") == 0 || std::strcmp(dbg, "true") == 0) {
                ...
                if (last_seqlen_k != seqlen_k) {
                    ...
                    fprintf(stderr,
                            "[INFINIOP][flash_attention] splitkv auto(mode): sm=%d heads=%zu seqs=%zu seqlen_k~%zu -> num_splits=%d\n",
    ...
    if (use_gqa_fused && use_cta && !use_split && alibi_slopes == nullptr && num_kv_heads > 0 &&
            ...
            if (const char *dbg = std::getenv("INFINIOP_FLASH_DEBUG_SPLITS")) {
                if (std::strcmp(dbg, "1") == 0 || std::strcmp(dbg, "true") == 0) {
                    ...
                    if (last_seqlen_k != seqlen_k) {
                        last_seqlen_k = seqlen_k;
                        fprintf(stderr,
                                "[INFINIOP][flash_attention] splitkv auto: sm=%d heads=%zu seqs=%zu seqlen_k~%zu -> num_splits=%d\n",
                                sm_count, num_heads, num_seqs, seqlen_k, num_splits);
                    }
                }
            }
