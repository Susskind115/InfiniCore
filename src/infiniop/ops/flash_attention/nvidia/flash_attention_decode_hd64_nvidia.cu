...
INFINIOP_CUDA_KERNEL flashAttentionDecodeHd64Warp(
    ...
    ptrdiff_t o_stride) {
    op::flash_attention::cuda::flashAttentionDecodeWarpKernel<Tindex, Tdata, 64>(
...
INFINIOP_CUDA_KERNEL flashAttentionDecodeHd64Cta(
    ...
    op::flash_attention::cuda::flashAttentionDecodeCtaKernel<Tindex, Tdata, 64, 32, 8>(
        out, q, k_cache, v_cache, block_tables, cache_lens, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size, q_stride,
        k_batch_stride, k_row_stride, k_head_stride, v_batch_stride, v_row_stride,
        v_head_stride, o_stride);
...
INFINIOP_CUDA_KERNEL flashAttentionDecodeHd64CtaTile16(
    ...
    ptrdiff_t v_head_stride,
    ptrdiff_t o_stride) {
    op::flash_attention::cuda::flashAttentionDecodeCtaKernel<Tindex, Tdata, 64, 32, 16>(
        out, q, k_cache, v_cache, block_tables, cache_lens, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size, q_stride,
        k_batch_stride, k_row_stride, k_head_stride, v_batch_stride, v_row_stride,
        v_head_stride, o_stride);
...
INFINIOP_CUDA_KERNEL flashAttentionDecodeHd64SplitKv(
    ...
    ptrdiff_t v_head_stride,
    int num_splits) {
    op::flash_attention::cuda::flashAttentionDecodeSplitKvWarpKernel<Tindex, Tdata, 64>(
...
INFINIOP_CUDA_KERNEL flashAttentionDecodeHd64SplitKvCombine(
    ...
    int num_splits,
    ptrdiff_t o_stride) {
    op::flash_attention::cuda::flashAttentionDecodeSplitKvCombineWarpKernel<Tdata, 64>(
    ...
    if (use_split) {
        if (use_cta) {
            ...
            if (!warned) {
                warned = true;
                fprintf(stderr,
                        "[INFINIOP][flash_attention] split-kv is enabled; ignoring INFINIOP_FLASH_DECODE_KERNEL=cta "
