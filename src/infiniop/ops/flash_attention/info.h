...
public:
    ...
    static utils::Result<FlashAttentionInfo> create(
        ...
        const bool block_tables_ok = (block_tables_dt == INFINI_DTYPE_I64) ||
                                     (block_tables_dt == INFINI_DTYPE_I32) ||
                                     (block_tables_dt == INFINI_DTYPE_U32);
        const bool cache_lens_ok = (cache_lens_dt == INFINI_DTYPE_I64) ||
                                  (cache_lens_dt == INFINI_DTYPE_I32) ||
                                  (cache_lens_dt == INFINI_DTYPE_U32);
        ...
        (void)num_blocks;
        const size_t page_block_size = k_shape[1];
        const size_t num_kv_heads = k_shape[2];
        ...
        if (v_cache_desc->shape()[0] != k_shape[0] || v_cache_desc->shape()[1] != k_shape[1] ||
            v_cache_desc->shape()[2] != k_shape[2] || v_cache_desc->shape()[3] != k_shape[3]) {
            return INFINI_STATUS_BAD_TENSOR_SHAPE;
        }
        ...
        const ptrdiff_t k_row_stride = k_cache_desc->stride(1);
        const ptrdiff_t k_head_stride = k_cache_desc->stride(2);

        const ptrdiff_t v_batch_stride = v_cache_desc->stride(0);
        const ptrdiff_t v_row_stride = v_cache_desc->stride(1);
        const ptrdiff_t v_head_stride = v_cache_desc->stride(2);
