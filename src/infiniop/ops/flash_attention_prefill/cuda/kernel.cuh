#ifndef __FLASH_ATTENTION_PREFILL_KERNEL_CUH__
#define __FLASH_ATTENTION_PREFILL_KERNEL_CUH__
...
namespace op::flash_attention_prefill::cuda {
...
__device__ void flashAttentionPrefillWarpKernel(
    Tdata *out_,
    const Tdata *q_,
...
#pragma unroll
            ...
            qk = op::flash_attention::cuda::warpReduceSum(qk);
...
template <typename Tindex, typename Tdata, int HEAD_SIZE>
__global__ void flashAttentionPrefillWarpGlobalKernel(
    Tdata *out_,
    const Tdata *q_,
    const Tdata *k_cache_,
    const Tdata *v_cache_,
...
#pragma unroll
    ...
    for (int logical_block = 0; t_base < allowed_k_len; ++logical_block, t_base += pbs) {
        ...
        const Tdata *k_base = k_cache_ + static_cast<int64_t>(phys) * k_batch_stride +
                              static_cast<int64_t>(kv_head_idx) * k_head_stride;
        const Tdata *v_base = v_cache_ + static_cast<int64_t>(phys) * v_batch_stride +
...
__global__ void flashAttentionPrefillReferenceKernel(
    Tdata *out_,
    const Tdata *q_,
    const Tdata *k_cache_,
    const Tdata *v_cache_,
    ...
    for (size_t t = 0; t <= causal_limit; ++t) {
        ...
        const Tdata *k_vec = k_cache_ + static_cast<int64_t>(phys) * k_batch_stride +
    ...
    for (size_t t = 0; t <= causal_limit; ++t) {
        ...
        const Tdata *v_vec = v_cache_ + static_cast<int64_t>(phys) * v_batch_stride +
...
__device__ void flashAttentionPrefillWarpCtaKernel(
    Tdata *out_,
    const Tdata *q_,
    const Tdata *k_cache_,
    const Tdata *v_cache_,
...
#pragma unroll
    ...
    for (int k_base = 0; k_base < max_allowed_k_len; k_base += BLOCK_N) {
        ...
        for (int idx = threadIdx.x; idx < tile_elems; idx += blockDim.x) {
            ...
            const Tdata *k_base_ptr = k_cache_ + static_cast<int64_t>(phys) * k_batch_stride +
                                      static_cast<int64_t>(off) * k_row_stride +
                                      static_cast<int64_t>(kv_head_idx) * k_head_stride;
            const Tdata *v_base_ptr = v_cache_ + static_cast<int64_t>(phys) * v_batch_stride +
                                      static_cast<int64_t>(off) * v_row_stride +
                                      static_cast<int64_t>(kv_head_idx) * v_head_stride;
...
__device__ void flashAttentionPrefillWarpCtaKernelPipelined(
    Tdata *out_,
    const Tdata *q_,
    const Tdata *k_cache_,
    const Tdata *v_cache_,
...
#pragma unroll
    ...
    for (int logical_block = 0; t_base < max_allowed_k_len; ++logical_block, t_base += pbs) {
        ...
        const Tdata *k_base = k_cache_ + static_cast<int64_t>(physical_block) * k_batch_stride +
                              static_cast<int64_t>(kv_head_idx) * k_head_stride;
        const Tdata *v_base = v_cache_ + static_cast<int64_t>(physical_block) * v_batch_stride +
                              static_cast<int64_t>(kv_head_idx) * v_head_stride;
        ...
        for (int ti = 0; ti < preload; ++ti) {
            ...
            for (int li = tid; li < LOADS_PER_TILE; li += blockDim.x) {
                ...
                if (tok < tile_n) {
                    ...
                    op::flash_attention::cuda::cpAsyncCaSharedGlobal16(&sh_k[ti][tok][off], k_src);
                    op::flash_attention::cuda::cpAsyncCaSharedGlobal16(&sh_v[ti][tok][off], v_src);
                } else {
                    reinterpret_cast<uint4 *>(&sh_k[ti][tok][off])[0] = make_uint4(0, 0, 0, 0);
                    reinterpret_cast<uint4 *>(&sh_v[ti][tok][off])[0] = make_uint4(0, 0, 0, 0);
                }
            }
            op::flash_attention::cuda::cpAsyncCommit();
            ++pending_groups;
        }

        int desired_pending = pending_groups - 1;
        if (desired_pending < 0) desired_pending = 0;
        if (desired_pending > (STAGES - 1)) desired_pending = (STAGES - 1);
        op::flash_attention::cuda::cpAsyncWaitGroupRt(desired_pending);
...
#pragma unroll
	                ...
	                float tile_m_tmp = op::flash_attention::cuda::warpReduceMax(score_lane);
	                ...
	                float sumexp_tmp = op::flash_attention::cuda::warpReduceSum(w_lane);
...
#pragma unroll
	...
	#pragma unroll
            ...
            if (prefetch_tile < num_tiles) {
                ...
                for (int li = tid; li < LOADS_PER_TILE; li += blockDim.x) {
                    ...
                    if (tok < prefetch_n) {
                        ...
                        op::flash_attention::cuda::cpAsyncCaSharedGlobal16(&sh_k[buf][tok][off], k_src);
                        op::flash_attention::cuda::cpAsyncCaSharedGlobal16(&sh_v[buf][tok][off], v_src);
                    } else {
            ...
            if (tile_idx + 1 < num_tiles) {
                ...
                if (desired_pending2 < 0) desired_pending2 = 0;
                if (desired_pending2 > (STAGES - 1)) desired_pending2 = (STAGES - 1);
                op::flash_attention::cuda::cpAsyncWaitGroupRt(desired_pending2);
                pending_groups = desired_pending2;
                __syncthreads();
            }
        }

        op::flash_attention::cuda::cpAsyncWaitAll();
        __syncthreads();
    }
...
template <typename Tindex, typename Tdata, int HEAD_SIZE, int BLOCK_M, int TOKENS_PER_TILE, int STAGES>
__device__ void flashAttentionPrefillWarpCtaKernelPipelinedSplitKv(
    ...
    const int m_start = m_block * BLOCK_M;
    const int q_token_local = m_start + warp_id;
    if (m_start >= q_len) return; // uniform
    const bool is_active = (q_token_local < q_len);
    ...
    int64_t q_token = q_start;
    if (is_active) q_token += static_cast<int64_t>(q_token_local);
...
#pragma unroll
                ...
                if (lane < tile_n) max_tmp = sh_scores[warp_id][lane];
                max_tmp = op::flash_attention::cuda::warpReduceMax(max_tmp);
                ...
                float acc_tile[DIMS_PER_THREAD];
#pragma unroll
                for (int i = 0; i < DIMS_PER_THREAD; ++i) acc_tile[i] = 0.0f;
                if (tile_sumexp > 0.0f) {
                    for (int j = 0; j < tile_n; ++j) {
                        const float wj = __shfl_sync(0xffffffff, w_lane, j);
                        const Tdata *v_ptr = &sh_v[buf][j][0];
...
__device__ void flashAttentionPrefillSplitKvCombineWarpKernel(
    Tdata *out_,
...
template <typename Tindex, typename Tdata, int HEAD_SIZE, int BLOCK_M, int BLOCK_N>
__device__ void flashAttentionPrefillWarpCtaKernelKOnly(
    Tdata *out_,
    const Tdata *q_,
    const Tdata *k_cache_,
    const Tdata *v_cache_,
...
#pragma unroll
    ...
    for (int k_base = 0; k_base < max_allowed_k_len; k_base += BLOCK_N) {
        ...
        for (int t = 0; t < tile_n; ++t) {
            ...
            const int32_t phys = s_phys[t];
            const int32_t off = s_off[t];
            const Tdata *v_ptr = v_cache_ + static_cast<int64_t>(phys) * v_batch_stride +
                                 static_cast<int64_t>(off) * v_row_stride +
                                 static_cast<int64_t>(kv_head_idx) * v_head_stride;
...
__device__ __forceinline__ void flashAttentionPrefillMmaScoreUpdateRow(
    int lane,
    int k_base,
    int allowed_k_len,
    ...
    for (int t = lane; t < kBlockN; t += kWarpSize) {
        const int kpos = k_base + t;
        if (kpos >= allowed_k_len) continue;
    ...
    float tile_m = op::flash_attention::cuda::warpReduceMax(local_max);
...
#pragma unroll
    ...
    float tile_sumexp = op::flash_attention::cuda::warpReduceSum(sumexp_lane);
...
__device__ __forceinline__ void flashAttentionPrefillMmaScoreWriteRow(
    int lane,
    bool active,
    int q_token_local,
    int64_t q_start,
...
__device__ void flashAttentionPrefillWarpCta8MmaHd128Kernel(
    half *out_,
    const half *q_,
    const half *k_cache_,
    const half *v_cache_,
    ...
    const Tindex *block_table =
        block_tables_ + static_cast<int64_t>(seq_idx) * static_cast<int64_t>(block_table_batch_stride);
    ...
    for (int k_base = 0; k_base < max_allowed_k_len; k_base += kBlockN) {
        ...
        for (int idx = threadIdx.x; idx < kBlockN * kHeadDim; idx += blockDim.x) {
            ...
            if (phys >= 0) {
                const int32_t off = s_off[t];
                const half *k_ptr =
                    k_cache_ + static_cast<int64_t>(phys) * k_batch_stride +
                    static_cast<int64_t>(off) * k_row_stride +
                    static_cast<int64_t>(kv_head_idx) * k_head_stride;
                const half *v_ptr =
                    v_cache_ + static_cast<int64_t>(phys) * v_batch_stride +
                    static_cast<int64_t>(off) * v_row_stride +
                    static_cast<int64_t>(kv_head_idx) * v_head_stride;
...
#pragma unroll
        ...
        if (row0 < kBlockM) {
            flashAttentionPrefillMmaScoreUpdateRow<kWarpSize, kBlockN, kHeadDim, kDimsPerThread>(
    ...
    // Write outputs.
    if (row0 < kBlockM) {
        flashAttentionPrefillMmaScoreWriteRow<kWarpSize, kHeadDim, kDimsPerThread>(
...
#endif // __FLASH_ATTENTION_PREFILL_KERNEL_CUH__
