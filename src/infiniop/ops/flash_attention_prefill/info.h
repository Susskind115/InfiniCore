#ifndef __FLASH_ATTENTION_PREFILL_INFO_H__
#define __FLASH_ATTENTION_PREFILL_INFO_H__
...
namespace op::flash_attention_prefill {

class FlashAttentionPrefillInfo {
    FlashAttentionPrefillInfo() = default;
...
public:
    ...
    static utils::Result<FlashAttentionPrefillInfo> create(
        ...
        if (alibi_slopes_desc.has_value()) {
        ...
        // Support both common paged-KV physical layouts:
        // - token-major: [num_blocks, page_block_size, kv_heads, head_dim]
        // - head-major : [num_blocks, kv_heads, page_block_size, head_dim] (used by current PagedKVCache)
        //
        // We infer layout by which dimension is divisible by 256 (page_block_size must satisfy that).
        const size_t dim1 = k_shape[1];
        const size_t dim2 = k_shape[2];
        const bool dim1_is_block = (dim1 % 256 == 0);
        const bool dim2_is_block = (dim2 % 256 == 0);
        if (!dim1_is_block && !dim2_is_block) {
            return INFINI_STATUS_BAD_TENSOR_SHAPE;
        }
        // If both are divisible by 256 (unlikely), prefer token-major to match FA2 docs.
        const bool token_major = dim1_is_block;

        const size_t page_block_size = token_major ? dim1 : dim2;
        const size_t num_kv_heads = token_major ? dim2 : dim1;
        ...
        if (token_major) {
            if (v_shape[1] != page_block_size || v_shape[2] != num_kv_heads) {
                return INFINI_STATUS_BAD_TENSOR_SHAPE;
            }
        } else {
        ...
        const ptrdiff_t k_row_stride = token_major ? k_cache_desc->stride(1) : k_cache_desc->stride(2);
        const ptrdiff_t k_head_stride = token_major ? k_cache_desc->stride(2) : k_cache_desc->stride(1);

        const ptrdiff_t v_batch_stride = v_cache_desc->stride(0);
        const ptrdiff_t v_row_stride = token_major ? v_cache_desc->stride(1) : v_cache_desc->stride(2);
        const ptrdiff_t v_head_stride = token_major ? v_cache_desc->stride(2) : v_cache_desc->stride(1);
        ...
        if (const char *dbg = std::getenv("INFINIOP_DEBUG_PREFILL_INFO")) {
            ...
            if (!printed && std::strcmp(dbg, "1") == 0) {
                ...
                std::fprintf(stderr,
                             "[infiniop][flash_attention_prefill][info] k_layout=%s k_shape=[%zu,%zu,%zu,%zu] k_strides=[%td,%td,%td,%td] (row_stride=%td head_stride=%td)\n",
                             token_major ? "token-major" : "head-major",
        ...
        return utils::Result<FlashAttentionPrefillInfo>(FlashAttentionPrefillInfo{
            dtype,
            block_tables_dt,
            scale,
            num_seqs,
...
#endif // __FLASH_ATTENTION_PREFILL_INFO_H__
