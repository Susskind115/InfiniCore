...
// #include "flash_attention_prefill_fa2.cuh"
#include "flash_attention_prefill_nvidia.cuh"

namespace op::flash_attention_prefill::nvidia {
...
inline const char *default_prefill_kernel(const FlashAttentionPrefillInfo &info) {
    ...
    if (info.page_block_size == 256 && (info.dtype == INFINI_DTYPE_F16 || info.dtype == INFINI_DTYPE_BF16)) {
        if (info.head_size == 128) return "warpcta8pipe";
...
INFINIOP_CUDA_KERNEL flashAttentionPrefillHd128Warp(
    Tdata *out,
    const Tdata *q,
    const Tdata *k_cache,
    const Tdata *v_cache,
    ...
    op::flash_attention_prefill::cuda::flashAttentionPrefillWarpKernel<Tindex, Tdata, 128>(
...
template <typename Tindex, typename Tdata>
INFINIOP_CUDA_KERNEL flashAttentionPrefillHd64Warp(
    Tdata *out,
    const Tdata *q,
    const Tdata *k_cache,
    const Tdata *v_cache,
    ...
    ptrdiff_t o_head_stride) {
    // Legacy per-seq launch (kept only as a wrapper; current "warp" impl uses a global-token kernel).
    op::flash_attention_prefill::cuda::flashAttentionPrefillWarpKernel<Tindex, Tdata, 64>(
        out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size, block_table_batch_stride,
        q_stride, q_head_stride, k_batch_stride, k_row_stride, k_head_stride,
        v_batch_stride, v_row_stride, v_head_stride, o_stride, o_head_stride);
...
template <typename Tindex, typename Tdata>
INFINIOP_CUDA_KERNEL flashAttentionPrefillHd128WarpCta(
    Tdata *out,
    const Tdata *q,
    const Tdata *k_cache,
    const Tdata *v_cache,
    ...
    op::flash_attention_prefill::cuda::flashAttentionPrefillWarpCtaKernel<Tindex, Tdata, 128, 4, 64>(
        out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
...
template <typename Tindex, typename Tdata>
INFINIOP_CUDA_KERNEL flashAttentionPrefillHd64WarpCta(
    Tdata *out,
    const Tdata *q,
    const Tdata *k_cache,
    const Tdata *v_cache,
    ...
    ptrdiff_t o_head_stride) {
    // 4 warps per CTA, one warp per query token.
    op::flash_attention_prefill::cuda::flashAttentionPrefillWarpCtaKernel<Tindex, Tdata, 64, 4, 128>(
        out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
        block_table_batch_stride,
        q_stride, q_head_stride,
...
template <typename Tindex, typename Tdata>
INFINIOP_CUDA_KERNEL flashAttentionPrefillHd128WarpCta8(
    Tdata *out,
    const Tdata *q,
    const Tdata *k_cache,
    const Tdata *v_cache,
    ...
    op::flash_attention_prefill::cuda::flashAttentionPrefillWarpCtaKernel<Tindex, Tdata, 128, 8, 64>(
        out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
        block_table_batch_stride,
        q_stride, q_head_stride,
...
template <typename Tindex, typename Tdata>
INFINIOP_CUDA_KERNEL flashAttentionPrefillHd128WarpCta8N128(
    Tdata *out,
    const Tdata *q,
    const Tdata *k_cache,
    const Tdata *v_cache,
    ...
    op::flash_attention_prefill::cuda::flashAttentionPrefillWarpCtaKernelKOnly<Tindex, Tdata, 128, 8, 128>(
        out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
        block_table_batch_stride,
        q_stride, q_head_stride,
...
template <typename Tindex, typename Tdata>
INFINIOP_CUDA_KERNEL flashAttentionPrefillHd64WarpCta8(
    Tdata *out,
    const Tdata *q,
    const Tdata *k_cache,
    const Tdata *v_cache,
    ...
    ptrdiff_t o_head_stride) {
    // 8 warps per CTA, one warp per query token.
    op::flash_attention_prefill::cuda::flashAttentionPrefillWarpCtaKernel<Tindex, Tdata, 64, 8, 128>(
        out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
        block_table_batch_stride,
        q_stride, q_head_stride,
...
template <typename Tindex, typename Tdata>
INFINIOP_CUDA_KERNEL flashAttentionPrefillHd128WarpCta8Pipe(
    Tdata *out,
    const Tdata *q,
    const Tdata *k_cache,
    const Tdata *v_cache,
    ...
    op::flash_attention_prefill::cuda::flashAttentionPrefillWarpCtaKernelPipelined<Tindex, Tdata, 128, 8, 32, 2>(
        out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
        block_table_batch_stride,
        q_stride, q_head_stride,
...
INFINIOP_CUDA_KERNEL flashAttentionPrefillHd128WarpCta8Mma(
    half *out,
    const half *q,
    const half *k_cache,
    const half *v_cache,
    ...
    ptrdiff_t o_stride,
    ptrdiff_t o_head_stride) {
    op::flash_attention_prefill::cuda::flashAttentionPrefillWarpCta8MmaHd128Kernel<Tindex>(
        out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
        block_table_batch_stride,
        q_stride, q_head_stride,
...
template <typename Tindex, typename Tdata>
INFINIOP_CUDA_KERNEL flashAttentionPrefillHd64WarpCta8Pipe(
    Tdata *out,
    const Tdata *q,
    const Tdata *k_cache,
    const Tdata *v_cache,
    ...
    ptrdiff_t o_head_stride) {
    // 8 warps per CTA, one warp per query token, with cp.async pipelining.
    op::flash_attention_prefill::cuda::flashAttentionPrefillWarpCtaKernelPipelined<Tindex, Tdata, 64, 8, 32, 2>(
        out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
        block_table_batch_stride,
        q_stride, q_head_stride,
...
template <typename Tindex, typename Tdata>
INFINIOP_CUDA_KERNEL flashAttentionPrefillHd128WarpCta8PipeSplitKv(
    float *partial_acc,
    float *partial_m,
    float *partial_l,
    int num_splits,
    ...
    op::flash_attention_prefill::cuda::flashAttentionPrefillWarpCtaKernelPipelinedSplitKv<Tindex, Tdata, 128, 8, 32, 2>(
...
template <typename Tindex, typename Tdata>
INFINIOP_CUDA_KERNEL flashAttentionPrefillHd64WarpCta8PipeSplitKv(
    float *partial_acc,
    float *partial_m,
    float *partial_l,
    int num_splits,
    ...
    const int split_idx = bz / num_m_blocks;
    const int m_block = bz - split_idx * num_m_blocks;
    op::flash_attention_prefill::cuda::flashAttentionPrefillWarpCtaKernelPipelinedSplitKv<Tindex, Tdata, 64, 8, 32, 2>(
        partial_acc, partial_m, partial_l, split_idx, num_splits, m_block, total_q_tokens,
        q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
        block_table_batch_stride,
...
INFINIOP_CUDA_KERNEL flashAttentionPrefillHd128SplitKvCombine(
    Tdata *out,
    ...
    ptrdiff_t o_stride,
    ptrdiff_t o_head_stride) {
    op::flash_attention_prefill::cuda::flashAttentionPrefillSplitKvCombineWarpKernel<Tdata, 128>(
        out, partial_acc, partial_m, partial_l, num_splits, total_q_tokens, o_stride, o_head_stride);
}

template <typename Tdata>
INFINIOP_CUDA_KERNEL flashAttentionPrefillHd64SplitKvCombine(
    Tdata *out,
    const float *partial_acc,
    const float *partial_m,
    const float *partial_l,
    ...
    ptrdiff_t o_stride,
    ptrdiff_t o_head_stride) {
    op::flash_attention_prefill::cuda::flashAttentionPrefillSplitKvCombineWarpKernel<Tdata, 64>(
        out, partial_acc, partial_m, partial_l, num_splits, total_q_tokens, o_stride, o_head_stride);
}

template <typename Tindex, typename Tdata>
INFINIOP_CUDA_KERNEL flashAttentionPrefillHd128WarpCta16(
    Tdata *out,
    const Tdata *q,
    const Tdata *k_cache,
    const Tdata *v_cache,
    ...
    op::flash_attention_prefill::cuda::flashAttentionPrefillWarpCtaKernel<Tindex, Tdata, 128, 16, 64>(
        out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
        block_table_batch_stride,
        q_stride, q_head_stride,
...
template <typename Tindex, typename Tdata>
INFINIOP_CUDA_KERNEL flashAttentionPrefillHd64WarpCta16(
    Tdata *out,
    const Tdata *q,
    const Tdata *k_cache,
    const Tdata *v_cache,
    ...
    ptrdiff_t o_head_stride) {
    // 16 warps per CTA, one warp per query token.
    op::flash_attention_prefill::cuda::flashAttentionPrefillWarpCtaKernel<Tindex, Tdata, 64, 16, 128>(
        out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
        num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
        block_table_batch_stride,
        q_stride, q_head_stride,
...
infiniStatus_t launch_prefill_ref(
    ...
    if (head_size == 64) {
        op::flash_attention_prefill::cuda::flashAttentionPrefillReferenceKernel<Tindex, Tdata, Tcompute, 64>
            <<<grid, block, 0, stream>>>(
                out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
    ...
    if (head_size == 128) {
        op::flash_attention_prefill::cuda::flashAttentionPrefillReferenceKernel<Tindex, Tdata, Tcompute, 128>
            <<<grid, block, 0, stream>>>(
                out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
                num_heads, num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
                block_table_batch_stride, q_stride, q_head_stride,
...
infiniStatus_t launch_prefill_warp(
    ...
    // - matches FlashAttention varlen (cu_seqlens) mental model better
    ...
    switch (head_size) {
    case 64:
        op::flash_attention_prefill::cuda::flashAttentionPrefillWarpGlobalKernel<Tindex, Tdata, 64>
            <<<grid, block, 0, stream>>>(
                out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
        ...
        return INFINI_STATUS_SUCCESS;
    case 128:
        op::flash_attention_prefill::cuda::flashAttentionPrefillWarpGlobalKernel<Tindex, Tdata, 128>
            <<<grid, block, 0, stream>>>(
                out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
                num_heads, num_seqs, num_kv_heads, total_q_tokens, scale, max_num_blocks_per_seq,
                page_block_size, block_table_batch_stride,
...
infiniStatus_t launch_prefill(
    ...
    switch (head_size) {
    case 64:
        flashAttentionPrefillHd64WarpCta<Tindex, Tdata>
            <<<grid, block, 0, stream>>>(
                out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
                num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
                block_table_batch_stride,
        ...
        return INFINI_STATUS_SUCCESS;
    case 128:
        flashAttentionPrefillHd128WarpCta<Tindex, Tdata>
            <<<grid, block, 0, stream>>>(
                out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
                num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
                block_table_batch_stride,
...
infiniStatus_t launch_prefill_warpcta8(
    ...
    switch (head_size) {
    case 64:
        flashAttentionPrefillHd64WarpCta8<Tindex, Tdata>
            <<<grid, block, 0, stream>>>(
                out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
                num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
                block_table_batch_stride,
        ...
        return INFINI_STATUS_SUCCESS;
    case 128:
        flashAttentionPrefillHd128WarpCta8<Tindex, Tdata>
            <<<grid, block, 0, stream>>>(
                out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
                num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
                block_table_batch_stride,
...
infiniStatus_t launch_prefill_warpcta8pipe(
    ...
    switch (head_size) {
    case 64:
        flashAttentionPrefillHd64WarpCta8Pipe<Tindex, Tdata>
            <<<grid, block, 0, stream>>>(
                out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
                num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
                block_table_batch_stride,
        ...
        return INFINI_STATUS_SUCCESS;
    case 128:
        flashAttentionPrefillHd128WarpCta8Pipe<Tindex, Tdata>
            <<<grid, block, 0, stream>>>(
                out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
                num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
                block_table_batch_stride,
...
infiniStatus_t launch_prefill_warpcta8mma(
    ...
    if (!force_mma && seqlen_k_est > 4096) {
        ...
        if (!warned) {
            std::fprintf(stderr,
                         "[infiniop][flash_attention_prefill] warpcta8mma is experimental and very slow for long seqlen_k (est=%zu). "
    ...
    flashAttentionPrefillHd128WarpCta8Mma<Tindex>
        <<<grid, block, 0, stream>>>(
...
infiniStatus_t launch_prefill_warpcta8pipe_splitkv(
    ...
    switch (head_size) {
    case 64:
        flashAttentionPrefillHd64WarpCta8PipeSplitKv<Tindex, Tdata>
            <<<grid, block, 0, stream>>>(
        ...
        break;
    case 128:
        flashAttentionPrefillHd128WarpCta8PipeSplitKv<Tindex, Tdata>
            <<<grid, block, 0, stream>>>(
                partial_acc, partial_m, partial_l, num_splits, total_q_tokens,
                q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
                num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
    ...
    switch (head_size) {
    case 64:
        flashAttentionPrefillHd64SplitKvCombine<Tdata>
            <<<grid2, block2, 0, stream>>>(
                out, partial_acc, partial_m, partial_l, num_splits, total_q_tokens, o_stride, o_head_stride);
        return INFINI_STATUS_SUCCESS;
    case 128:
        flashAttentionPrefillHd128SplitKvCombine<Tdata>
            <<<grid2, block2, 0, stream>>>(
                out, partial_acc, partial_m, partial_l, num_splits, total_q_tokens, o_stride, o_head_stride);
        return INFINI_STATUS_SUCCESS;
    default:
...
infiniStatus_t launch_prefill_warpcta8n128(
    ...
    flashAttentionPrefillHd128WarpCta8N128<Tindex, Tdata>
        <<<grid, block, 0, stream>>>(
            out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
            num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
            block_table_batch_stride,
...
infiniStatus_t launch_prefill_warpcta16(
    ...
    switch (head_size) {
    case 64:
        flashAttentionPrefillHd64WarpCta16<Tindex, Tdata>
            <<<grid, block, 0, stream>>>(
                out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
                num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
                block_table_batch_stride,
        ...
        return INFINI_STATUS_SUCCESS;
    case 128:
        flashAttentionPrefillHd128WarpCta16<Tindex, Tdata>
            <<<grid, block, 0, stream>>>(
                out, q, k_cache, v_cache, block_tables, total_kv_lens, cu_seqlens_q, alibi_slopes,
                num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
                block_table_batch_stride,
...
infiniStatus_t Descriptor::create(
    ...
    auto info = FlashAttentionPrefillInfo::create(
    ...
    const size_t splitkv_workspace_bytes =
        use_splitkv ? (static_cast<size_t>(num_splits) * n * (info->head_size + 2) * sizeof(float)) : 0;
...
#define DISPATCH_SPLITKV(Tindex, Tdata, BT_PTR)                                                       \
    return launch_prefill_warpcta8pipe_splitkv<Tindex, Tdata>(                                        \
        ...
        if (_info.dtype == INFINI_DTYPE_F16) {
            if (_info.index_dtype == INFINI_DTYPE_I64) { DISPATCH_SPLITKV(int64_t, half, block_tables); }
            if (_info.index_dtype == INFINI_DTYPE_I32) { DISPATCH_SPLITKV(int32_t, half, block_tables); }
            if (_info.index_dtype == INFINI_DTYPE_U32) { DISPATCH_SPLITKV(uint32_t, half, block_tables); }
            return INFINI_STATUS_BAD_TENSOR_DTYPE;
        }
        if (_info.dtype == INFINI_DTYPE_BF16) {
            if (_info.index_dtype == INFINI_DTYPE_I64) { DISPATCH_SPLITKV(int64_t, __nv_bfloat16, block_tables); }
            if (_info.index_dtype == INFINI_DTYPE_I32) { DISPATCH_SPLITKV(int32_t, __nv_bfloat16, block_tables); }
            if (_info.index_dtype == INFINI_DTYPE_U32) { DISPATCH_SPLITKV(uint32_t, __nv_bfloat16, block_tables); }
            return INFINI_STATUS_BAD_TENSOR_DTYPE;
        }
        return INFINI_STATUS_BAD_TENSOR_DTYPE;
...
#define DISPATCH_KERNEL(Tindex, Tdata, Tcompute)                                                          \
    do {                                                                                                  \
        ...
        if (k_env != nullptr) {                                                                           \
            const bool known =                                                                            \
                (std::strcmp(k, "warp") == 0) || (std::strcmp(k, "warpcta") == 0) ||                       \
                (std::strcmp(k, "warpcta8") == 0) || (std::strcmp(k, "warpcta8pipe") == 0) ||             \
                (std::strcmp(k, "warpcta8mma") == 0) || (std::strcmp(k, "warpcta8n128") == 0) ||          \
                (std::strcmp(k, "warpcta16") == 0) ||                  \
                (std::strcmp(k, "ref") == 0);                                                             \
            if (!known) {                                                                                 \
                const char *fallback = default_prefill_kernel(_info);                                     \
                std::fprintf(stderr,                                                                      \
                             "[infiniop][flash_attention_prefill] WARNING: unknown kernel '%s', falling back to '%s'\n", \
                             k, fallback);                                                                \
                k = fallback;                                                                             \
            }                                                                                             \
        }                                                                                                 \
        ...
        if (!printed_dispatch && dbg != nullptr && std::strcmp(dbg, "1") == 0) {                          \
            std::fprintf(stderr,                                                                          \
                         "[infiniop][flash_attention_prefill] kernel=%s (override=%s head_size=%zu block=%zu dtype=%zu)\n", \
                         k,                                                                               \
